...
# READING CSV AND EXCEL FILES TO DETERMING THE TOTAL COUNT
...
server <- function(input, output) {
  ...
  output$box_1 <- renderValueBox({
    ...
    valueBox(
      totalInterviews, "Total Longitudinal Survey", icon = icon("users"),
      color = "blue"
    )
  })
  output$box_2 <- renderValueBox({
    ...
    valueBox(
      totalInterviews, "Total Men Interviews ", icon = icon("users"),
      color = "blue"
    )
  })
  output$box_3 <- renderValueBox({
    ...
    valueBox(
      totalInterviews, "Total Cross Sectional Interviews", icon = icon("users"),
      color = "blue"
    )
  })
  output$box_4 <- renderValueBox({
    ...
    valueBox(
      totalInterviews, "Total Health Facility Survey", icon = icon("users"),
      color = "blue"
    )
  })
  output$box_5 <- renderValueBox({
    ...
    valueBox(
      totalInterviews, "Total Womens Interviews ", icon = icon("users"),
      color = "blue"
    )
  })
